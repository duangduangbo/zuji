...
//		lat=Number(lat)+i;
//		lot=Number(lot)+i;
//		var point1 = new plus.maps.Point(lat,lot);
...
//		pointsAll.push(point3);
		...
		if(pointsArr.length>=2){
			...
			console.log(JSON.stringify(pointsArr));
 ...
 w.clearWatch=function(map,arr) {
    ...
    console.log(p);
		...
		if(p[0].latitude==p[p.length-1].latitude && p[0].latitude==p[(Math.floor(p.length-1)/2)].latitude){
			...
			var btnArray = ['否', '是'];
			mui.confirm('添加轨迹，确认？', '足迹', btnArray, function(e) {
			if (e.index == 1) {
		//将坐标转换为地址
				...
				var url="http://192.168.43.189:88/zujidata/add_route.php";
			      	mui.ajax(url,{
			      		...
			      		data: {
			      			...
			      			dtime:arr.dtime,
			      			uname:arr.uname,
			      			points:JSON.stringify(linePoints),
			      			rstart:rstart,
			      			rend:rend
			      		},
			      		crossDomain:true,
...
//显示覆盖物
w.showMarker=function(pos,map,title,did,dminutes,url,range){
	var marker=new plus.maps.Marker(pos);
	...
	bubble.setIcon("/logo.png");
	marker.setBubble(bubble,true);
	var range=range?range:400;
	var circle=new plus.maps.Circle(pos,range);
	circle.setStrokeOpacity(0.8);
	...
	plus.maps.Map.reverseGeocode(pos,{},function(event){
		 ...
		 mui.openWindow({
			id: 'show_view.html',
			url:url==undefined?'html/show_view.html?did='+did:url,
			show: {
			aniShow: 'pop-in'
			},
			waiting: {
